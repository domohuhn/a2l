...
VariantNaming variantNamingFromString(Token s) {
  switch(s.text) {
    case 'ALPHA'  : return VariantNaming.ALPHA  ;
    case 'NUMERIC': return VariantNaming.NUMERIC;
    default: throw ParsingException('Unknown VariantNaming scheme', s);
  }
}
...
String variantNamingToString(VariantNaming s) {
  switch(s) {
    case VariantNaming.ALPHA  : return 'ALPHA'  ;
    case VariantNaming.NUMERIC: return 'NUMERIC';
    default: throw ValidationError('Unknown VariantNaming scheme $s');
  }
}
  ...
  String toFileContents(int depth) {
    ...
    if(measurement!=null) {
      if(measurement!.isEmpty || measurement!.contains(' ')) {
        throw ValidationError('Identifiers must not be empty or conatin spaces! VAR_MEASUREMENT "$measurement" in VariantEnumeration $name');
      }
      rv += indent('VAR_MEASUREMENT $measurement',depth+1);
    }
    if(characteristic!=null) {
      if(characteristic!.isEmpty || characteristic!.contains(' ')) {
        throw ValidationError('Identifiers must not be empty or conatin spaces! VAR_SELECTION_CHARACTERISTIC "$measurement" in VariantEnumeration $name');
      }
...
class VariantCharacteristic {
  VariantCharacteristic() : enumerations = [], address=[];
...
class VariantCoding {
  VariantCoding() : enumerations = [], forbidden = [], characteristics=[];
