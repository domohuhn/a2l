...
SegmentType segmentTypeFromString(Token t) {
  switch (t.text) {
    case 'CALIBRATION_VARIABLES':
      return SegmentType.CALIBRATION_VARIABLES;
    case 'CODE':
      return SegmentType.CODE;
    case 'DATA':
      return SegmentType.DATA;
    case 'EXCLUDE_FROM_FLASH':
      return SegmentType.EXCLUDE_FROM_FLASH;
    case 'OFFLINE_DATA':
      return SegmentType.OFFLINE_DATA;
    case 'RESERVED':
      return SegmentType.RESERVED;
    case 'SERAM':
      return SegmentType.SERAM;
    case 'VARIABLES':
      return SegmentType.VARIABLES;
    default:
...
String segmentTypeToString(SegmentType e) {
  switch (e) {
    case SegmentType.CALIBRATION_VARIABLES:
      return 'CALIBRATION_VARIABLES';
    case SegmentType.CODE:
      return 'CODE';
    case SegmentType.DATA:
      return 'DATA';
    case SegmentType.EXCLUDE_FROM_FLASH:
      return 'EXCLUDE_FROM_FLASH';
    case SegmentType.OFFLINE_DATA:
      return 'OFFLINE_DATA';
    case SegmentType.RESERVED:
      return 'RESERVED';
    case SegmentType.SERAM:
      return 'SERAM';
...
MemoryType memoryTypeFromString(Token t) {
  switch (t.text) {
    ...
    default:
      throw ParsingException('Unknown memory type', t);
  }
}
...
SegmentAttribute segmentAttributeFromString(Token t) {
  switch (t.text) {
    case 'INTERN':
      return SegmentAttribute.INTERN;
    case 'EXTERN':
      return SegmentAttribute.EXTERN;
    default:
      throw ParsingException('Unknown SegmentAttribute', t);
  }
}
...
class MemorySegment extends SegmentData {
  ...
  String toFileContents(int depth) {
    ...
    rv += indent(
        '${segmentTypeToString(type)} ${memoryTypeToString(memoryType)} ${segmentAttributeToString(attribute)}',
        depth + 1);
