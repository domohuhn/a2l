...
String processIncludes(String text, FileLoader loader, {int recursion = 0}) {
  if (recursion > loader.maxRecursion) {
    throw ValidationError(
        'Maximum file inclusion depth exceeded (current: $recursion > max: ${loader.maxRecursion})');
  }
  ...
  while (m != null) {
    ...
    if (file == null) {
      throw ValidationError(
          'Internal logic error: the match should always have a group! "$m"');
    }
    try {
      ...
      if (recursion == 0) {
        throw PreprocessingError(
            ...
            findLineOrColumn(text, m.start),
            findLineOrColumn(text, m.start, getColumn: true));
      } else {
        rethrow;
      }
    }
...
class _Preprocessor {
  ...
  List<UnprocessedSection> unprocessedSections = [
    UnprocessedSection(RegExp(r'/begin\s*IF_DATA'),
        RegExp(r'[\t\f\v ]*/end[\s]*IF_DATA', multiLine: true)),
    UnprocessedSection(RegExp(r'/begin\s*A2ML'),
        RegExp(r'[\t\f\v ]*/end[\s]*A2ML', multiLine: true))
  ];
  ...
  void parse() {
    ...
    for (var i = 0; i < content.length; i++) {
      if (startPassThrough == i) {
        final endPassThrough =
            _findEndOfCurrentUnprocessedSection(i, line, column);
        if (startPassThrough < endPassThrough &&
            endPassThrough < content.length) {
          if (token.text.isNotEmpty) {
            tokens.add(token);
            token = Token('', line, column, i);
          }
          tokens.add(Token(content.substring(startPassThrough, endPassThrough),
              line, column, i));
          ...
          throw PreprocessingError(
              'Size of unprocessed block is negative! Invariant start<end ($startPassThrough < $endPassThrough) or end<filesize ($endPassThrough < ${content.length}) does not hold!',
              line,
              column);
        }
      }
      ...
      if (!isStr && char == '"') {
        if (token.text.isNotEmpty) {
          throw PreprocessingError(
              'Missing whitespace at start of string!', line, column);
        }
        token.text = '';
        isStr = true;
      } else if (isStr && char == '"') {
        ...
        if (i + 1 < content.length && !whiteSpace.hasMatch(content[i + 1])) {
          throw PreprocessingError(
              'Missing whitespace after end of string!', line, column);
        }
      }
  ...
  int skipMultiLineComment(int start) {
    ...
    throw PreprocessingError(
        'Unterminated multiline comment', startLine, startCol);
  }
  ...
  void _incrementCounters(String c) {
    if (c.length != 1) {
      throw PreprocessingError(
          'Internal error: File must be parsed character by character, got "$c"',
          line,
          column);
    }
    column += 1;
